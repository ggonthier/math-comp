...
Lemma common_eigenvector n (As : seq 'M[C]_n) :
  (n > 0)%N -> {in As &, forall A B, commmx A B} ->
  exists2 v : 'rV_n, v != 0 & all (fun A => stablemx v A) As.
Proof.
move=> n_gt0 /all_commmxP; have [k sAsk] := ubnP (size As).
...
rewrite ltnS all_commmx_cons => sAsk /andP[].
move=> /allP/(_ _ _)/eqP/= A_comm /all_commmxP As_comm.
have [a a_eigen] := eigenvalue_closed A n_gt0.
have [] := IHk _ _ [seq restrictmx (eigenspace A a) B | B <- As].
- by rewrite lt0n mxrank_eq0.
- by rewrite size_map.
- apply/all_commmxP; move=> _ _ /= /mapP /= [B B_in ->] /mapP /= [B' B'_in ->].
  rewrite -?conjmxM ?inE ?stablemx_row_base ?commmx_stable_eigenspace//;
...
by rewrite commmx_stable_eigenspace //; apply: A_comm.
Qed.
...
Lemma cotrigonalization n (As : seq 'M[C]_n) :
  {in As &, forall A B, commmx A B} ->
  cotrigonalisable_in (@unitarymx n n) As.
Proof.
...
Theorem Schur n (A : 'M[C]_n) : (n > 0)%N -> trigonalisable_in (@unitarymx n n) A.
Proof.
